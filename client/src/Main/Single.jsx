...
export default function Single({ Geans, Title }) {
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_Free_NetFlix}?${Geans}`).then((res) => {
      datavalue(res.data);
    ...
    <>
      <div className="row">
        <div>
          ...
          <Link to={`/all_content/${Geans.split("1")[0]}/1/&_limit=50`}>
            <button className="MoreButton">
              <Text className="MoreText">
                More <ArrowRightIcon className="MoreArrow" />
              </Text>
            </button>
          </Link>
        </div>
        <div className="row_posters" ref={containerRef}>
          <button className="ScrollLeft" onClick={handelScrollLeft}>
            <ArrowLeftIcon />
          </button>
          {data.length === 0
            ? arr.map((Item) => {
                return (
                  <Image
                    src={process.env.REACT_APP_GRAY_COLOR}
            ...
            : data.map((Item, index) => {
                return (
                  ...
                  <Image
                    ...
                    className="row_poster"
                    // onClick={() => {
                    //   navigate(`/player/${Item.Title.split(" ").join("_")}/${Item.MainCategory}/${Item.Plateform}/${Item.FileID}/1/${Item.Image.split("/").join("---")}`);
                    // }}
          ...
          <button className="ScrollRight" onClick={handelScrollRight}>
            <ArrowRightIcon />
          </button>
        </div>
      </div>
